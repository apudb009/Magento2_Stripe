    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            if($payment->getAdditionalInformation('save_card')){
                $customer = \Stripe\Customer::create(['email' => $order->getCustomerEmail(), 'source' => $payment->getAdditionalInformation('source')]);
                if($customer->id){
                    $customer = \Stripe\Customer::retrieve($customer->id);
                    $customerCard = $customer->sources->create(['card' => $payment->getAdditionalInformation('token')]);

                    $charge = \Stripe\Charge::create([
                        'amount' => $amount * 100,
                        'currency' => strtolower($order->getBaseCurrencyCode()),
                        'customer' => $customer->id,
                        'source'  => $customerCard->id
                    ]);
                }
                $payment
